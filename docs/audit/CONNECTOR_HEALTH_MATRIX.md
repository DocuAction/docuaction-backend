...
| SAM.gov | Blocked Upstream | N/A | 2026-08-09T00:00:00+00:00 | **Excluded** | (see notes) | **Key proven valid.** The same key returns HTTP 200 with `X-Ratelimit-Limit: 36000` on api.data.gov (DEMO_KEY: 10, invalid key: 401). Every api.sam.gov path returns an empty HTTP 404 from `server: istio-envoy` — including with no key, an invalid key, the bare root, and paths that do not exist. Requests are refused at SAM's ingress before authentication. Not a code, credential or FOUO-privilege issue. |
