...
try:
    from . import gdelt_tv_ingest
    from . import gdelt_doc_ingest
    from . import reddit_ingest
    from . import cspan_fcc_ingest
    _FREE_SOURCES_AVAILABLE = True
except ImportError:
    _FREE_SOURCES_AVAILABLE = False
...
def is_federal_holiday(check_date):
    from datetime import date as _date
    holidays_2026 = [
        _date(2026,1,1),_date(2026,1,19),_date(2026,2,16),_date(2026,5,25),
        _date(2026,6,19),_date(2026,7,3),_date(2026,9,7),_date(2026,10,12),
        _date(2026,11,11),_date(2026,11,26),_date(2026,12,25),
    ]
    return check_date in holidays_2026, json, logging, asyncio, hashlib, re
...
<body>
<div style="display:flex;align-items:center;border-top:3px solid #0F2A4A;border-bottom:3px solid #0F2A4A;padding:16px 0;margin-bottom:20px;">
<img src="data:image/jpeg;base64," alt="FCC Seal" style="width:70px;height:70px;margin-right:20px;">
<div style="text-align:right;flex:1;">
<p class="date" style="margin:0;">{briefing_date}</p>
<p class="agency" style="margin:4px 0;">{agency.name}</p>
<p style="font-size:18px;font-weight:bold;color:#1F4E79;margin:4px 0;">Daily News Summary</p>
</div>
</div>
...
- The headline link MUST point to the story summary anchor below (#story-1, #story-2, etc.) so clicking jumps down to the detailed summary. This is a TABLE OF CONTENTS — it navigates within the document, not to external sites.
- Each article gets a unique anchor id: #story-1, #story-2, etc.
...
async def run_daily_cycle(
    ...
    # Holiday check — skip generation on federal holidays
    if is_federal_holiday(now_et().date()):
        return {"status": "skipped", "reason": f"{now_et().date()} is a federal holiday"}
    ...
    # DATE-BASED briefing ID — same date = same briefing (no duplicates)
    briefing_date_key = datetime.now().strftime('%Y%m%d')
    briefing_id = f"{agency_id}_{briefing_date_key}"
    
    # Check if briefing already exists for this date
    existing_briefing = _briefings.get(briefing_id)
    existing_articles = []
    if existing_briefing:
        existing_articles = existing_briefing.get("articles", [])
        existing_urls = {a.url for a in existing_articles if hasattr(a, 'url')}
        logger.info(f"Found existing briefing {briefing_id} with {len(existing_articles)} articles — will merge")
    ...
    # Update or create briefing (same date = update, not duplicate)
    _briefings[briefing_id] = briefing
    logger.info(f"Briefing {briefing_id}: {briefing['article_count']} articles ({'updated' if existing_briefing else 'new'})")
    try:
