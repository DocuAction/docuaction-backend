...
async def process_text(request: ProcessRequest, user=Depends(get_current_user)):
    ...
    Input: transcript or document text + action_type
    Output: structured JSON (summary, tasks, decisions, follow_ups)
    
    Action types: summary, actions, insights, email, brief
    
    Pipeline: PII Masking → Chunking → Model Routing → JSON Repair → Audit Log
    """
...
async def transcribe_audio(
    ...
    """
    Upload an audio file and transcribe using OpenAI Whisper.
...
async def generate_output(
    ...
    """Generate AI output from a document."""
    result = await db.execute(select(Document).where(Document.id == doc_id, Document.user_id == user.id))
    doc = result.scalar_one_or_none()
    if not doc:
        raise HTTPException(404, "Document not found")
    ...
    try:
        ...
        text = f"[Binary file: {doc.filename}]"
...
async def list_outputs(db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    """List all AI outputs for current user."""
