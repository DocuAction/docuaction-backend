...
TEMPLATES = {
    ...
    "MULTIPLE_CANDIDATE_ENTITIES": "{source} associates the delivered {system} with {count} distinct candidate entities ({reason}); the match is not unique.",
    "IDENTIFIER_CORROBORATED_MULTIPLE_RECORDS": "The delivered {system} resolves to one {source} entity (entity type: {entity_type}) stated in {count} source records; several records for one identifier are a property of the source dataset, not several entities. Identifier corroboration does not establish licensure or program compliance.",
    ...
    "ROLE_ASSIGNMENT_DIFFERS": "The delivered location ({delivered_role}) is the same address that {source} records with the role {role}. Same address, different role: the source does not state that care is delivered there, and the engine does not infer it.",
    "SOURCE_ROLE_UNKNOWN": "The delivered location ({delivered_role}) is the same address that {source} records without stating a role. Same address, unstated role: the engine does not assign a practice, administrative or mailing role the source did not state, so this address does not corroborate any role-specific question.",
    "RELATIONSHIP_PERIOD_DIFFERS": "{source} records the same {kind} relationship as delivered but states a different validity period (delivered {delivered_period}; source {source_period}). Different periods are not an automatic contradiction.",
    ...
    "DELTA_SOURCE_NO_LONGER_REPORTS": "The {what} that {source} reported in the prior edition is not reported in the current edition ('{before}'). This records a change in what the source reports; it does not establish the end of a relationship, the lapse of a registration, or any adverse fact.",
